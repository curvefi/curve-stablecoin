#!/usr/bin/env python3
"""
Deploy the sDOLA/crvUSD LlamaLend V2 market on Ethereum Mainnet.

Borrowed token: crvUSD.  Collateral: sDOLA (Inverse Finance ERC4626 vault).

Per market this deploys, in order:
    1. Oracle stack (prices sDOLA in USD):
         a. OracleFromCurvePools([pool], [borrowed_idx], [collateral_idx])
         b. CrvUSDAggregatorWrapper(pool_oracle)   -> x crvUSD/USD aggregator
         c. ERC4626EMAWrapper(agg_wrapper, sDOLA, ema_time)  -> x vault share price
    2. SDolaRateCalculator(sDOLA) -> per-second yield rate of the collateral.
    3. HyperbolicDynamicMP(controller, rate_calculator, curve params...).
    4. factory.create(crvUSD, sDOLA, ... oracle, monetary_policy, supply_limit).
...
boa-deploy-llamalend-mainnet-sDOLA-crvUSD.py --dry-run --account-name <name>

    # broadcast
    MAINNET_RPC_URL=... python scripts/mainnet-deployment/markets/\
boa-deploy-llamalend-mainnet-sDOLA-crvUSD.py --account-name <name>
"""
...
# --- Oracle config (from tmp/deploy-oracles.py: "sDOLA-crvUSD") ---
POOL = "0x76A962BA6770068bCF454D34dDE17175611e6637"  # sDOLA/scrvUSD
POOL_BORROWED_IDX = 0
...
RATE_CALCULATOR = "curve_stablecoin/mpolicies/v2/rate_calculators/SDolaRateCalculator.vy"
...
BORROW_CAP = 12_400_000 * 10**18  # crvUSD (18 decimals)
ADMIN_FEE = 10**17                # 10%
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Deploy LlamaLend V2 sDOLA/crvUSD on Mainnet"
    )
