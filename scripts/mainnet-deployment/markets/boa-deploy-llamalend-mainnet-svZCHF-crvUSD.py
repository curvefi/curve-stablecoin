#!/usr/bin/env python3
"""
Deploy the svZCHF/crvUSD LlamaLend V2 market on Ethereum Mainnet.

Borrowed token: crvUSD.  Collateral: svZCHF (Frankencoin ERC4626 savings vault).

Per market this deploys, in order:
    1. Oracle stack (prices svZCHF in USD).
         a. OracleFromCurvePools([pool], [borrowed_idx], [collateral_idx])
         b. CrvUSDAggregatorWrapper(pool_oracle)   -> x crvUSD/USD aggregator
         c. ERC4626EMAWrapper(agg_wrapper, svZCHF, ema_time)  -> x vault share price
    2. HyperbolicMP(controller, fixed target rate, curve params...).
    3. factory.create(crvUSD, svZCHF, ... oracle, monetary_policy, supply_limit).
...
boa-deploy-llamalend-mainnet-svZCHF-crvUSD.py --dry-run --account-name <name>

    # broadcast
    MAINNET_RPC_URL=... python scripts/mainnet-deployment/markets/\
boa-deploy-llamalend-mainnet-svZCHF-crvUSD.py --account-name <name>
"""
...
SVZCHF = "0xE5F130253fF137f9917C0107659A4c5262abf6b0"  # collateral (ERC4626 vault)
COLLATERAL = SVZCHF

# --- Oracle config (from results/ZCHF_CRVUSD/oracle.md) ---
POOL = "0x027B40F5917FCd0eac57d7015e120096A5F92ca9"  # crvUSD/ZCHF
POOL_BORROWED_IDX = 0
POOL_COLLATERAL_IDX = 1
# Smoothing horizon of the vault-share-price EMA (seconds). 866 ~= 600s / ln(2).
# 1 day - 124648 = 86400s / ln(2)
EMA_TIME = 124648
...
# --- Monetary policy curve (from results/ZCHF_CRVUSD/monetary_policy_config.json) ---
TARGET_UTILIZATION = 8 * 10**17  # 80%
TARGET_RATE = 1_673_572_355  # 5.2778% simple APR, per second and 1e18-scaled
LOW_RATIO = 3 * 10**17  # 0.3x target rate at 0% utilization
HIGH_RATIO = 10 * 10**18  # 10x target rate at 100% utilization
RATE_SHIFT = 0

# --- Market risk parameters (from results/ZCHF_CRVUSD/overview.md) ---
A = 180
FEE = int(0.0005 * 10**18)  # 0.05%
LOAN_DISCOUNT = int(0.043 * 10**18)  # 4.3%
LIQUIDATION_DISCOUNT = int(0.023 * 10**18)  # 2.3%
SUPPLY_LIMIT = 2**256 - 1  # unlimited; borrow cap set separately

# --- Post-create configuration (requires factory admin / DAO vote) ---
BORROW_CAP = 500_000 * 10**18  # crvUSD (18 decimals)
ADMIN_FEE = 10**17  # 10%
...
def _deploy(
    ...
    # 2. Fixed-rate monetary policy, bound to the controller create() will deploy.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Deploy LlamaLend V2 svZCHF/crvUSD on Mainnet"
    )
    ...
    parser.add_argument(
        "--report-path",
        default="deployments/mainnet/markets/llamalend-mainnet-svZCHF-crvUSD.jsonc",
