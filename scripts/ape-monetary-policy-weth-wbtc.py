...
RATE = int((1.06**(1 / (365 * 86400)) - 1) * 1e18)  # 6% if PegKeepers are empty, 2.2% when at target fraction
SIGMA = 2 * 10**16  # 2% when at target debt fraction
DEBT_FRACTION = 10 * 10**16  # 10%

MARKETS = {'WBTC': "0x4e59541306910ad6dc1dac0ac9dfb29bd9f15c67", 'ETH': "0xa920de414ea4ab66b97da1bfe9e6eca7d4219635"}
...
def deploy(network):
    ...
    with accounts.use_sender(account):
        for i, market in enumerate(MARKETS.keys()):
            ...
            print('Rate:', (1 + mpolicy.rate(MARKETS[market]) / 1e18) ** (365 * 86400) - 1)
            print()
