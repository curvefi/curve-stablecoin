...
Companion to deploy.py, which redeploys just these two
...
Verifies, with their constructor args:
    1. SfrxUSDRateCalculator (rate_calculator) -- ctor: (collateral)
    2. HyperbolicDynamicMP   (monetary_policy) -- ctor: (controller, rate_calculator,
                                                  target_util, low, high, rate_shift)

Run:
    ETHERSCAN_API_KEY=... python scripts/deploy/llamalend/ethereum/monetary_policies/\
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Verify sfrxUSD/crvUSD RateCalculator + HyperbolicDynamicMP on Etherscan"
    )
    parser.add_argument(
        "--deployment",
