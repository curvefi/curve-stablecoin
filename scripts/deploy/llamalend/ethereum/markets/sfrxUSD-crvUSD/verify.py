#!/usr/bin/env python3
"""Verify the sfrxUSD/crvUSD market's oracle stack, rate calculator, and monetary
...
and submits source + constructor args for each contract that script deploys:
    ...
    3. ERC4626EMAWrapper       (price_oracle)
    4. SfrxUSDRateCalculator   (rate_calculator)
    5. HyperbolicDynamicMP     (monetary_policy)

The vault/controller/amm are blueprint proxies deployed by the factory; their
implementations are verified by scripts/deploy/llamalend/ethereum/factory/verify.py.

Run:
    ETHERSCAN_API_KEY=... python scripts/deploy/llamalend/ethereum/markets/\
sfrxUSD-crvUSD/verify.py
"""
...
DEFAULT_DEPLOYMENT = "deployments/llamalend/ethereum/markets/sfrxUSD-crvUSD.jsonc"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Verify sfrxUSD/crvUSD market contracts on Etherscan"
    )
    parser.add_argument(
        "--deployment",
    ...
    contracts = [
        ...
        (
            ...
            encode(
                ...
                [agg_wrapper_addr, collateral, ema_time],
            ).hex(),
            "1",
        ),
        (
            rate_calculator_addr,
            "SfrxUSDRateCalculator (Vyper 0.4.3)",
