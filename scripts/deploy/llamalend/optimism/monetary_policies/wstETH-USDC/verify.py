#!/usr/bin/env python3
"""Verify the wstETH/USDC HyperbolicMP monetary policy on Optimism Etherscan.

Companion to deploy.py, which deploys just the
...
Run:
    ETHERSCAN_API_KEY=... python scripts/deploy/llamalend/optimism/monetary_policies/\
wstETH-USDC/verify.py
"""
...
# ---------------------------------------------------------------------------
# Contract source + curve params (must match deploy.py)
# ---------------------------------------------------------------------------
...
DEFAULT_DEPLOYMENT = "deployments/llamalend/optimism/markets/wstETH-USDC.jsonc"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Verify wstETH/USDC HyperbolicMP on Optimism Etherscan"
    )
    parser.add_argument(
        "--deployment",
