...
CHAINLINK_FEED = (
    "0x524299Ab0987a7c4B3c8022a35669DdcdC715a10"  # wstETH / WETH Chainlink feed
)
...
WSTETH_RATE_ORACLE = (
    "0x294ED1f214F4e0ecAE31C3Eae4F04EBB3b36C9d0"  # Lido TokenRateOracle (wstETH/stETH)
)
...
def _deploy(
    ...
    oracle = boa.load_partial_solc(
        "scripts/deploy/llamalend/optimism/solidity/ChainlinkEMA.sol"
    ).deploy(
        CHAINLINK_FEED,
        OBSERVATIONS,
        INTERVAL,
...
def main() -> None:
    ...
    parser.add_argument(
        "--factory-deployment",
        default="deployments/llamalend/optimism/factory.jsonc",
        help="Path to existing factory deployment JSON to read factory address from",
    )
    parser.add_argument(
        "--report-path",
        default="deployments/llamalend/optimism/markets/wstETH-WETH.jsonc",
