#!/usr/bin/env python3
"""Verify the LlamaLend V2 TransientLeverageZapLend on Optimism Etherscan."""
...
def main() -> None:
    ...
    deployment_path = (
        PROJECT_ROOT / "deployments" / "llamalend" / "optimism" / "transient-leverage-zap.jsonc"
    )
    deployment = json.loads(deployment_path.read_text())

    leverage_zap_addr = deployment["transient_leverage_zap"]
    ...
    label = "TransientLeverageZapLend (Vyper 0.4.3)"
    ...
    try:
        guid = _submit(
            api_key,
            address,
            "curve_stablecoin/zaps/transient_leverage_zap/TransientLeverageZapLend.vy:TransientLeverageZapLend",
            std_json,
            ctor_hex,
            "vyper:0.4.3",
            "vyper-json",
