...
def setup(network, account):
    ...
    with accounts.use_sender(account) as account:
        ...
        occupied_slot = address_provider.get_address(
        ...
        logger.info(
            f"Registry at AddressProvider Registry ID: {STABLESWAP_FACTORY_ADDRESS_PROVIDER_ID}: {occupied_slot}"
        )
        ...
        assert (
            address_provider.get_address(STABLESWAP_FACTORY_ADDRESS_PROVIDER_ID)
            == STABLESWAP_FACTORY
        )
        logger.info(
            f"Updated AddressProvider Registry ID: {STABLESWAP_FACTORY_ADDRESS_PROVIDER_ID} with {STABLESWAP_FACTORY}"
        )
        ...
        factory_handler = account.deploy(
            project.StableswapFactoryHandler,
            STABLESWAP_FACTORY,
            BASE_POOL_REGISTRY,
            **kw,
        )
        ...
        if not factory_handler_integrated:
            logger.info(
                "First integration ... adding factory handler to the metaregistry"
            )
