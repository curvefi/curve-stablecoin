...
def parse_functions(source_lines: list[str]) -> list[dict]:
    ...
    for func in funcs:
        ...
        sig_text = ' '.join(l.split('#', 1)[0] for l in source_lines[start:sig_end + 1])
...
def dummy_return(ret_type: str | None) -> str:
    ...
    if ret_type.startswith('(') and ret_type.endswith(')'):
        ...
        vals = []
        for p in parts:
            if '[' in p or p.startswith('DynArray') or p.startswith('Bytes') or p.startswith('String'):
    ...
    if '[' in ret_type or ret_type.startswith('DynArray') or ret_type.startswith('Bytes') or ret_type.startswith('String'):
