...
def test_deleverage(
    ...
    with chain.isolate():
        ...
        max_borrowable, _ = leverage_zaps[
            collateral_token
        ...
        if repay_debt_amount < borrow_amt:
            ...
            assert (
                abs((borrow_amt - debt) - repay_debt_amount) / repay_debt_amount < 1e-4
            )
            assert n1 == _n1
            assert n2 == _n1 + _N - 1
        else:
...
def test_deleverage_underwater(
    ...
    with chain.isolate():
        ...
        llammas[collateral_token].exchange_dy(
            0, 1, total_collateral_to_trade, 10**26, sender=admin
        )
