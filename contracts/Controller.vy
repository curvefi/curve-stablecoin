...
def __init__(
    ...
    MAX_FEE = min(unsafe_div(10**18 * MIN_TICKS_UINT, A), 10**17)
    ...
    if collateral_token == empty(address):
        ...
        _collateral_token = ERC20(staticcall Factory(msg.sender).collateral_token())
        _borrowed_token = ERC20(staticcall Factory(msg.sender).borrowed_token())
    else:
        # Stablecoin factory
        # _collateral_token is already set
        _borrowed_token = ERC20(staticcall Factory(msg.sender).stablecoin())

    COLLATERAL_TOKEN = _collateral_token
    BORROWED_TOKEN = _borrowed_token
    COLLATERAL_PRECISION = pow_mod256(10, 18 - staticcall _collateral_token.decimals())
    BORROWED_PRECISION = pow_mod256(10, 18 - staticcall _borrowed_token.decimals())

    SQRT_BAND_RATIO = isqrt(unsafe_div(10**36 * _A, unsafe_sub(_A, 1)))

    assert extcall _borrowed_token.approve(msg.sender, max_value(uint256), default_return_value=True)
...
def execute_callback(callbacker: address, callback_sig: bytes4,
    ...
    response: Bytes[64] = raw_call(
        callbacker,
        concat(callback_sig, abi_encode(user, stablecoins, collateral, debt, callback_args, callback_bytes)),
        max_outsize=64
    )
...
def repay_extended(callbacker: address, callback_args: DynArray[uint256,5], callback_bytes: Bytes[10**4] = b"",  _for: address = msg.sender):
    ...
    # Else - partial repayment -> deleverage, but only if we are not underwater
        ...
        n2: int256 = n1 + size
        extcall AMM.deposit_range(_for, cb.collateral, n1, n2)
        liquidation_discount: uint256 = self.liquidation_discount
        self.liquidation_discounts[_for] = liquidation_discount
