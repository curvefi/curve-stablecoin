...
SFRXETH: public(immutable(sfrxETH))
...
@internal
@view
def _ema_tvl() -> TVL[N_POOLS]:
    last_timestamp: uint256 = self.last_timestamp
    tvls: TVL[N_POOLS] = self.tvl
    ...
    for i in range(N_POOLS):
        tvls[i].ema = (tvls[i].ema * alpha + tvls[i].last * (10**18 - alpha)) / 10**18
        tvls[i].last = TRICRYPTO[i].totalSupply() * TRICRYPTO[i].virtual_price() / 10**18
...
def ema_tvl() -> uint256[N_POOLS]:
    tvls: TVL[N_POOLS] = self._ema_tvl()
    out: uint256[N_POOLS] = empty(uint256[N_POOLS])
    for i in range(N_POOLS):
        out[i] = tvls[i].ema
    return out
...
def _raw_price(tvls: TVL[N_POOLS], agg_price: uint256) -> uint256:
    ...
    p_staked: uint256 = STAKEDSWAP.price_oracle()  # d_eth / d_frxeth

    p_staked = min(p_staked, 10**18) * SFRXETH.pricePerShare() / 10**18  # d_eth / d_sfrxeth
