...
@notice Price oracle for pools which contain cryptos and crvUSD. It also uses crvUSD price aggregator
...
AGG: public(constant(address)) = 0x2e9e2A7AfEfE04b722eAe39ae70Dbce5411f0fb1
...
@view
def price() -> uint256:
    return self._raw_price() * StableAggregator(AGG).price() / 10**18


@external
def price_w() -> uint256:
    return self._raw_price() * StableAggregator(AGG).price_w() / 10**18
