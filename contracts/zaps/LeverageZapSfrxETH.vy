...
"""
@title sfrxETH crvUSD leverage zap
@author Curve.Fi
...
def _get_collateral(stablecoin: uint256, route_idx: uint256) -> uint256:
    frxeth_amt: uint256 = ROUTER.get_exchange_multiple_amount(self.routes[route_idx], self.route_params[route_idx], stablecoin, self.route_pools[route_idx])
    return SFRXETH.convertToShares(frxeth_amt)
...
def get_collateral(stablecoin: uint256, route_idx: uint256) -> uint256:
    """
    @notice Calculate the expected amount of sfrxETH by given stablecoin amount
...
def get_collateral_underlying(stablecoin: uint256, route_idx: uint256) -> uint256:
    """
    @notice Calculate the expected amount of frxETH by given stablecoin amount
    @param stablecoin Amount of stablecoin
    @param route_idx Index of the route to use
...
def callback_deposit(user: address, stablecoins: uint256, collateral: uint256, debt: uint256, callback_args: DynArray[uint256, 5]) -> uint256[2]:
    ...
    frxeth: uint256 = ROUTER.exchange_multiple(self.routes[route_idx], self.route_params[route_idx], debt, min_recv, self.route_pools[route_idx])
    leverage_collateral: uint256 = SFRXETH.deposit(frxeth, self)
