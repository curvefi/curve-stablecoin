...
@internal
@pure
def _log_2(_x: uint256) -> uint256:
    """
    @dev An `internal` helper function that returns the log in base 2
         of `_x`, following the selected rounding direction.
...
def wad_ln(_x: uint256) -> int256:
    ...
    @notice Note that it returns 0 if given 0. Furthermore, this function
            ...
            of `_x`. The implementation is inspired by Remco Bloemen's
    ...
    """
    value: int256 = convert(_x, int256)

    assert _x > 0

    # We want to convert `_x` from "10 ** 18" fixed point to "2 ** 96"
    # fixed point. We do this by multiplying by "2 ** 96 / 10 ** 18".
    # But since "ln(_x * C) = ln(_x) + ln(C)" holds, we can just do nothing
    # here and add "ln(2 ** 96 / 10 ** 18)" at the end.

    # Reduce the range of `_x` to "(1, 2) * 2 ** 96".
    # Also remember that "ln(2 ** k * _x) = k * ln(2) + ln(_x)" holds.
    k: int256 = unsafe_sub(convert(self._log_2(_x), int256), 96)
...
@internal
@view
def _get_k_effective(_controller: address, _collateral: uint256, _N: uint256) -> uint256:
    """
    ...
    d_k_effective: uint256 = 10**18 * unsafe_sub(
        10**18, min(discount + (DEAD_SHARES * 10**18) / max(_collateral / _N, DEAD_SHARES), 10**18)
    ) / (SQRT_BAND_RATIO * _N)
...
@internal
@view
def _max_p_base(_controller: address) -> uint256:
    """
    @notice Calculate max base price including skipping bands
    @param _controller Controller of the market
    @return Max base price
    """
    AMM: LLAMMA = LLAMMA(Controller(_controller).amm())
    A: uint256 = AMM.A()
...
@external
@view
def max_borrowable(_controller: address, _user_collateral: uint256, _leverage_collateral: uint256, _N: uint256, _p_avg: uint256) -> uint256:
    """
    ...
    @param _N Number of bands the deposit is made into
    @param _p_avg Average price of collateral in borrowed token expected from swap
    ...
    max_p_base: uint256 = self._max_p_base(_controller)
    max_borrowable: uint256 = user_collateral * 10**18 / (10**36 / k_effective * 10**18 / max_p_base - 10**36 / _p_avg)

    return min(max_borrowable * 999 / 1000, ERC20(BORROWED_TOKEN).balanceOf(_controller)) # Cannot borrow beyond the amount of coins Controller has


@internal
def _approve(_coin: address, _spender: address):
    if ERC20(_coin).allowance(self, _spender) == 0:
        assert ERC20(_coin).approve(_spender, max_value(uint256), default_return_value=True)
...
@external
@nonreentrant('lock')
def callback_deposit(_user: address, _stablecoins: uint256, _user_collateral: uint256, _d_debt: uint256,
                     _callback_args: DynArray[uint256, 10], _callback_bytes: Bytes[10**4] = b"") -> uint256[2]:
    """
    @notice Callback method which should be called by controller to create leveraged position
    @param _user Address of the user
    @param _stablecoins Always 0 (unused)
    ...
    @param _callback_args Unused, kept for controller compatibility
    @param _callback_bytes ABI-encoded (controller_id, min_recv, exchange_address, exchange_calldata)
    ...
    controller_id, min_recv, exchange_address, exchange_calldata = _abi_decode(
        _callback_bytes, (uint256, uint256, address, Bytes[10 ** 4 - 160 - 16])
    )
    ...
    # Dust cleaning
    self._transfer(collateral_token, _user, ERC20(collateral_token).balanceOf(self))
    ...
    self._transfer(borrowed_token, _user, ERC20(borrowed_token).balanceOf(self))

    log Deposit(controller, _user, leverage_collateral, _d_debt)
...
@external
@nonreentrant('lock')
def callback_repay(_user: address, _stablecoins: uint256, _collateral: uint256, _debt: uint256,
                   _callback_args: DynArray[uint256,10], _callback_bytes: Bytes[10 ** 4] = b"") -> uint256[2]:
    """
    @notice Callback method for controller to deleverage/repay a position using collateral
    @param _user Address of the user
    @param _stablecoins The value from user_state (unused)
    @param _collateral The value from user_state (unused)
    @param _debt The value from user_state (unused)
    @param _callback_args Unused, kept for controller compatibility
    @param _callback_bytes ABI-encoded (controller_id, min_recv, exchange_address, exchange_calldata)
                          - controller_id is needed to check that msg.sender is the one of our controllers
    ...
    log Repay(controller, _user, state_collateral_used, borrowed_from_state_collateral)
