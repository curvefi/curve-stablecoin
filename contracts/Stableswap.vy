...
def add_liquidity(
    ...
    for i: uint256 in range(N_COINS):
        amount: uint256 = _amounts[i]
        if amount > 0:
            assert extcall IERC20(self.coins[i]).transferFrom(msg.sender, self, amount, default_return_value=True)  # dev: failed transfer
            new_balances[i] += amount
        else:
...
def remove_liquidity(
    ...
    for i: uint256 in range(N_COINS):
        ...
        amounts[i] = value
        assert extcall IERC20(self.coins[i]).transfer(_receiver, value, default_return_value=True)  # dev: failed transfer
...
def remove_liquidity_imbalance(
    ...
    for i: uint256 in range(N_COINS):
        ...
        if amount != 0:
            new_balances[i] -= amount
            assert extcall IERC20(self.coins[i]).transfer(_receiver, amount, default_return_value=True)  # dev: failed transfer
...
def commit_new_fee(_new_fee: uint256):
    assert msg.sender == staticcall Factory(self.factory).admin()
...
def withdraw_admin_fees():
    ...
    for i: uint256 in range(N_COINS):
        ...
        assert extcall IERC20(coin).transfer(receiver, fees, default_return_value=True)
