...
def test_leverage(
    ...
    max_borrowable, max_collateral = leverage_zaps[
        collateral_token
    ].max_borrowable_and_collateral(_collateral_amt, N, route_idx)
    ...
    if _borrow_amt == max_borrowable:
        ...
        expected_collateral = _collateral_amt + leverage_zaps[
            collateral_token
        ].get_collateral(_borrow_amt, route_idx)
    _n1 = leverage_zaps[collateral_token].calculate_debt_n1(
        _collateral_amt, _borrow_amt, N, route_idx
    )
    min_recv = int(
        leverage_zaps[collateral_token].get_collateral_underlying(
            _borrow_amt, route_idx
        )
        * 0.999
    )
