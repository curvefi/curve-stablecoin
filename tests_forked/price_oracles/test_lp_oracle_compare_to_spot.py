...
def test_tricrypto_usdc(lp_oracle_factory, stablecoin_aggregator, admin, trader):
    ...
    with boa.env.prank(admin):
        tricrypto_usdc_crvusd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(tricrypto_usdc_pool_address, usdc_crvusd_oracle.address)[1])  # USDC/LP * crvUSD/USDC
        tricrypto_usdc_usd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(tricrypto_usdc_pool_address, usdc_usd_oracle.address)[1])  # USDC/LP * crvUSD/USDC * USD/crvUSD
    ...
    with boa.env.prank(trader):
        ...
        prices_to_check = [int(p * initial_wbtc_spot_price) for p in [0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 0.92, 0.95, 0.98, 1.00, 1.02, 1.05, 1.08, 1.10, 1.20, 1.30, 1.40, 1.50, 1.60, 1.80, 2.00]]
        for target_p in prices_to_check:
            while tricrypto_usdc_pool.get_dy(1, 0, 10 ** 4) > target_p:
                tricrypto_usdc_pool.exchange(1, 0, wbtc.balanceOf(tricrypto_usdc_pool) // 500, 0)
                boa.env.time_travel(3600)

            while tricrypto_usdc_pool.get_dy(1, 0, 10 ** 4) < target_p:
                tricrypto_usdc_pool.exchange(0, 1, usdc.balanceOf(tricrypto_usdc_pool) // 500, 0)
                boa.env.time_travel(3600)
            ...
            delta = 1e-3
            print("p =", target_p / 10**2, "delta =", abs(lp_spot_price_usd - lp_oracle_price_usd) / lp_oracle_price_usd * 100, "%", "<", delta * 100, "%")
            assert abs(lp_spot_price_crvusd - lp_oracle_price_crvusd) / lp_oracle_price_crvusd < delta
            assert abs(lp_spot_price_usd - lp_oracle_price_usd) / lp_oracle_price_usd < delta
...
def test_tricrypto_usdt(lp_oracle_factory, stablecoin_aggregator, admin, trader):
    ...
    with boa.env.prank(admin):
        tricrypto_usdt_crvusd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(tricrypto_usdt_pool_address, usdt_crvusd_oracle.address)[1])  # USDT/LP * crvUSD/USDT
        tricrypto_usdt_usd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(tricrypto_usdt_pool_address, usdt_usd_oracle.address)[1])  # USDT/LP * crvUSD/USDT * USD/crvUSD

    # --- Compare oracle and spot prices ---

    # USDT interface is different (does not return bool from 'approve' method, for example)
    usdt = boa.from_etherscan(tricrypto_usdt_pool.coins(0), "USDT", uri=EXPLORER_URL, api_key=EXPLORER_TOKEN)
    wbtc = boa.load_partial("contracts/testing/ERC20Mock.vy").at(tricrypto_usdt_pool.coins(1))
    ...
    with boa.env.prank(trader):
        ...
        for target_p in prices_to_check:
            while tricrypto_usdt_pool.get_dy(1, 0, 10 ** 4) > target_p:
                tricrypto_usdt_pool.exchange(1, 0, wbtc.balanceOf(tricrypto_usdt_pool) // 500, 0)
                boa.env.time_travel(3600)

            while tricrypto_usdt_pool.get_dy(1, 0, 10 ** 4) < target_p:
                tricrypto_usdt_pool.exchange(0, 1, usdt.balanceOf(tricrypto_usdt_pool) // 500, 0)
                boa.env.time_travel(3600)
...
def test_tricrv(lp_oracle_factory, stablecoin_aggregator, admin, trader):
    ...
    with boa.env.prank(admin):
        tricrv_crvusd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(tricrv_pool_address, "0x0000000000000000000000000000000000000000")[1])  # USDT/LP * crvUSD/USDT
        tricrv_usd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(tricrv_pool_address, stablecoin_aggregator.address)[1])  # USDT/LP * crvUSD/USDT * USD/crvUSD

    # --- Compare oracle and spot prices ---

    crvusd = boa.load_partial("contracts/testing/ERC20Mock.vy").at(tricrv_pool.coins(0))
    weth = boa.load_partial("contracts/testing/WETH.vy").at(tricrv_pool.coins(1))
    ...
    with boa.env.prank(trader):
        ...
        prices_to_check = [int(p * initial_weth_spot_price) for p in [0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 0.92, 0.95, 0.98, 1.00, 1.02, 1.05, 1.08, 1.10, 1.20, 1.30, 1.40, 1.50, 1.60, 1.80, 2.00]]
        for target_p in prices_to_check:
            while tricrv_pool.get_dy(1, 0, 10 ** 12) > target_p:
                tricrv_pool.exchange(1, 0, boa.env.get_balance(tricrv_pool.address) // 500, 0)
                boa.env.time_travel(3600)
            ...
            print("p =", target_p / 10**12, "delta =", abs(lp_spot_price_usd - lp_oracle_price_usd) / lp_oracle_price_usd * 100, "%", "<", delta * 100, "%")
            assert abs(lp_spot_price_crvusd - lp_oracle_price_crvusd) / lp_oracle_price_crvusd < delta
            assert abs(lp_spot_price_usd - lp_oracle_price_usd) / lp_oracle_price_usd < delta
...
def test_strategic_reserve(lp_oracle_factory, stablecoin_aggregator, admin, trader):
    ...
    with boa.env.prank(admin):
        strategic_reserve_crvusd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(strategic_reserve_pool_address, usdc_crvusd_oracle.address)[1])  # USDC/LP * crvUSD/USDC
        strategic_reserve_usd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(strategic_reserve_pool_address, usdc_usd_oracle.address)[1])  # USDC/LP * crvUSD/USDC * USD/crvUSD
    ...
    # USDT interface is different (does not return bool from 'approve' method, for example)
    usdt = boa.from_etherscan(strategic_reserve_pool.coins(1), "USDT", uri=EXPLORER_URL, api_key=EXPLORER_TOKEN)
    ...
    with boa.env.prank(trader):
        ...
        prices_to_check = [p * 10**16 for p in [40, 50, 60, 70, 80, 90, 92, 95, 98, 100, 102, 105, 108, 110, 120, 130, 140, 150, 160, 180, 200]]
        for target_p in prices_to_check:
            while strategic_reserve_pool.get_p(0) > target_p:
                strategic_reserve_pool.exchange(1, 0, usdt.balanceOf(strategic_reserve_pool) // 500, 0)
                boa.env.time_travel(3600)

            while strategic_reserve_pool.get_p(0) < target_p:
                strategic_reserve_pool.exchange(0, 1, usdc.balanceOf(strategic_reserve_pool) // 500, 0)
                boa.env.time_travel(3600)
            ...
            print("p =", target_p / 10**18, "delta =", abs(lp_spot_price_usd - lp_oracle_price_usd) / lp_oracle_price_usd * 100, "%", "<", delta * 100, "%")
            assert abs(lp_spot_price_crvusd - lp_oracle_price_crvusd) / lp_oracle_price_crvusd < delta
            assert abs(lp_spot_price_usd - lp_oracle_price_usd) / lp_oracle_price_usd < delta
...
def test_weeth_weth(lp_oracle_factory, stablecoin_aggregator, admin, trader):
    ...
    with boa.env.prank(admin):
        weeth_ng_pool_crvusd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(weeth_ng_pool_address, usdt_crvusd_oracle.address)[1])  # ETH/LP * crvUSD/ETH
        weeth_ng_pool_usd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(weeth_ng_pool_address, usdt_usd_oracle.address)[1])  # ETH/LP * crvUSD/ETH * USD/crvUSD

    # --- Compare oracle and spot prices ---

    weth = boa.load_partial("contracts/testing/WETH.vy").at(weeth_ng_pool.coins(0))
    weeth = boa.load_partial("contracts/testing/ERC20Mock.vy").at(weeth_ng_pool.coins(1))
    ...
    with boa.env.prank(trader):
        ...
        for target_p in prices_to_check:
            ...
            lp_spot_price_usd = lp_spot_price_crvusd * stablecoin_aggregator.price() // 10 ** 18
...
def test_cvxcrv(lp_oracle_factory, stablecoin_aggregator, admin, trader):
    ...
    with boa.env.prank(admin):
        cvxcrv_pool_crvusd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(cvxcrv_pool_address, crv_crvusd_oracle)[1])  # CRV/LP * crvUSD/CRV
        cvxcrv_pool_usd_lp_oracle = boa.load_partial('contracts/price_oracles/proxy/ProxyOracle.vy').at(
            lp_oracle_factory.deploy_oracle(cvxcrv_pool_address, crv_usd_oracle)[1])  # CRV/LP * crvUSD/CRV * USD/crvUSD
