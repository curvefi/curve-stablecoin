"""Property-based (hypothesis) layer for HyperbolicMP.

These sit on top of the example-based unit tests in
tests/unitary/mpolicies/hyperbolic_mp and share the same pure-Python reference
...
# These params pass all the assertions in HyperbolicMP._set_parameters()
...
# Fixed base rate; validated into [MIN_TARGET_RATE, MAX_TARGET_RATE] on-chain.
target_rate_st = st.integers(
    min_value=ref.MIN_TARGET_RATE, max_value=ref.MAX_TARGET_RATE
)
...
@given(
    params=valid_params(),
    target_rate=target_rate_st,
    reserves=reserves_st,
    frac=st.integers(min_value=0, max_value=WAD),
    fee_frac=fee_frac_st,
)
@settings(max_examples=1000, deadline=None)
def test_matches_reference(params, target_rate, reserves, frac, fee_frac):
    ...
    mp, ctrl = _deploy(u0, low, high, shift, target_rate)

    # parameters depend only on the curve args + the fixed target rate
    p = mp.parameters()
    assert (
        p.u_inf,
        p.A,
        ...
        p.high_ratio,
        p.rate_shift,
    ) == (u_inf, A, r_minf, u0, target_rate, low, high, shift)

    # rate() depends on the fixed target rate and the controller state
...
def test_params_correct_or_curve_revert(params):
    ...
    u0, low, high, shift = params
    try:
        mp, _ = _deploy(u0, low, high, shift, ref.DEFAULT_RATE)
...
def test_rate_monotonic_in_utilization(params, reserves, f1, f2, fee_frac):
    ...
    mp, ctrl = _deploy(u0, low, high, shift, ref.DEFAULT_RATE)
...
@settings(max_examples=500, deadline=None)
def test_rate_survives_any_admin_fees(params, target_rate, reserves, frac, g1, g2):
    ...
    assume(ref.calculate_rate(ref_params, u_lo, target_rate, shift) >= 0)
    assume(ref.calculate_rate(ref_params, u_hi, target_rate, shift) >= 0)
    ...
    assert rate_lo == ref.calculate_rate(ref_params, u_lo, target_rate, shift)
    assert rate_hi == ref.calculate_rate(ref_params, u_hi, target_rate, shift)
    assert rate_hi >= rate_lo
...
@given(scenario=state_and_action(), target_rate=target_rate_st)
@settings(max_examples=1000, deadline=None)
def test_future_rate_matches_applied_action(scenario, target_rate):
