...
def test_dydx_limits(amm, swap_calc, amounts, accounts, ns, dns, collateral_token, admin, borrowed_token):
    ...
    borrowed_precision = 10**(max(borrowed_deciamls - 14, 0))  # 18 - 10_000, 17 - 1000, 16 - 100, 15 - 10, < 14 - 1
...
@settings(deadline=timedelta(seconds=1000))
def test_exchange_by_dy_down_up(amm, swap_calc, amounts, accounts, ns, dns, amount, borrowed_token, collateral_token, admin):
    collateral_deciamls = collateral_token.decimals()
    borrowed_deciamls = borrowed_token.decimals()
    amounts = list(map(lambda x: int(x * 10**collateral_deciamls), amounts))
    ...
    dy, dx = swap_calc.get_dydx(amm, 0, 1, amount)
    assert dy <= amount
    dy2, dx2 = swap_calc.get_dydx(amm, 0, 1, dy)
    assert dy == dy2
    assert approx(dx, dx2, 1e-6)
    borrowed_token._mint_for_testing(u, dx2)
    with boa.env.prank(u):
        swap_calc.exchange_by_dy(amm, 0, 1, dy2, 0)  # crvUSD --> ETH
    ...
    with boa.env.prank(u):
        swap_calc.exchange_by_dy(amm, 1, 0, out_amount, 0)  # ETH --> crvUSD
