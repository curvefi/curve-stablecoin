...
@pytest.fixture(scope="module")
def stableswap_b(
    unsafe_factory, swap_deployer, swap_impl_ng, stablecoin, stablecoin_b, admin
):
    with boa.env.prank(admin):
...
def provide_token_to_peg_keepers_no_sleep(
    ...
    for (amount_r, amount_s), swap, pk, rtoken in zip(
        ...
        with boa.env.prank(alice):
            ...
            assert swap.balances(0) == pytest.approx(
                swap.balances(1) // rtoken_mul, rel=1e-6
            )
...
def imbalance_pool(
    ...
    def _inner(swap, i, amount=None, add_diff=False):
        with boa.env.prank(alice):
            ...
            if add_diff:
                amount += (
                    swap.balances(1 - i) * token_mul[1 - i]
