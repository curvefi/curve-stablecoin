...
def test_default_behavior(blueprint_registry):
    ...
    blueprint_registry.set("AMM", boa.env.generate_address())
    blueprint_registry.set("CTR", boa.env.generate_address())
...
def test_invalid_id(blueprint_registry):
    ...
    with boa.reverts(dev="blueprint id not allowed"):
        blueprint_registry.set("XXXX", boa.env.generate_address())
