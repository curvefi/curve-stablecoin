...
def test_ctor_emits_set_parameters(deployer, controller, rate_calculator, default_params):
    ...
    u_inf, A, r_minf = ref.get_params(*default_params)
    assert (
        e.u_inf, e.A, e.r_minf,
        e.target_utilization, e.low_ratio, e.high_ratio, e.rate_shift,
    ) == (u_inf, A, r_minf) + default_params + (0,)
...
@pytest.mark.parametrize(
    ("args", "err"),
    [
        ((ref.MIN_TARGET_UTIL - 1, 5 * 10**17, 2 * 10**18, 0), "target_utilization too low"),
        ((ref.MAX_TARGET_UTIL + 1, 5 * 10**17, 2 * 10**18, 0), "target_utilization too high"),
        ...
        ((85 * 10**16, 5 * 10**17, 2 * 10**18, ref.MAX_RATE_SHIFT + 1), "rate_shift too high"),
