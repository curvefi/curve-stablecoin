...
`_get_utilization` and `_calculate_rate` together through the public view. The
base rate is the fixed `target_rate` parameter (no EMA).
"""
...
def test_rate_matches_reference(
    ...
    assert mp.rate() == ref.calculate_rate(params, u, target_rate)


def test_rate_zero_debt(mp, controller, default_curve, target_rate):
    controller.set_state(0, 100 * 10**18, 0)
    # At u = 0 the rate reduces to r0*r_minf/1e18 + A*r0/u_inf + shift
    params = ref.get_params(*default_curve)
    assert mp.rate() == ref.calculate_rate(params, 0, target_rate)
...
def test_rate_scales_with_target_rate(deployer, controller, default_curve):
    # A higher fixed target_rate yields a proportionally higher curve.
    ...
    assert low.rate() == ref.calculate_rate(params, u, ref.DEFAULT_RATE)
    assert high.rate() == ref.calculate_rate(params, u, 3 * ref.DEFAULT_RATE)
    assert high.rate() > low.rate()


def test_rate_with_shift(deployer, controller, default_curve, target_rate):
    # A non-zero rate_shift adds a flat term to the curve.
    shift = 5 * 10**9
    u0, alpha, beta = default_curve
    mp = deployer.deploy(controller.address, u0, target_rate, alpha, beta, shift)
    controller.set_state(50 * 10**18, 50 * 10**18, 0)

    params = ref.get_params(*default_curve)
    u = ref.utilization(50 * 10**18, 50 * 10**18, 0)
    assert mp.rate() == ref.calculate_rate(params, u, target_rate, shift)
