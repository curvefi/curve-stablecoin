...
def test_max_borrowable(borrowed_token, collateral_token, amm, controller, price_oracle, admin,
    ...
    if max_borrowable >= total:
        return
    with boa.reverts():
        controller.calculate_debt_n1(collateral_amount, int(max_borrowable * 1.001) + n, n)
    if max_borrowable == 0:
        return
...
@given(
    ...
    f_p_o=st.floats(min_value=0.7, max_value=1.3))
@settings(max_examples=1000)
def test_min_collateral(borrowed_token, collateral_token, amm, controller, price_oracle, admin,
                        debt_amount, n, f_p_o):
    # Create some liquidity and go into the band
    with boa.env.prank(admin):
        c_amount = 10**collateral_token.decimals()
