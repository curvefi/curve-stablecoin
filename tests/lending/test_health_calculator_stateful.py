...
class StatefulLendBorrow(RuleBasedStateMachine):
    ...
    def __init__(self):
        ...
        self.preexisting_supply = (
            self.borrowed_token.totalSupply()
            - self.borrowed_token.balanceOf(self.controller)
        )
    ...
    def health_calculator(self, user, d_collateral, d_amount):
        if self.controller.loan_exists(user):
            ...
            try:
                future_health = self.controller.health_calculator(
                    user, d_collateral, d_amount, False
                )
                future_health_full = self.controller.health_calculator(
            ...
            try:
                ...
                assert self.controller.health(user) == pytest.approx(
                    future_health, rel=1e-4, abs=1e18 / debt
                )
                assert self.controller.health(user, True) == pytest.approx(
                    future_health_full, rel=1e-4, abs=1e18 / debt
                )
    ...
    def create_loan(self, c_amount, amount_frac, n, user_id):
        ...
        amount = (
            min(int(amount_frac * c_amount * 3000), self.debt_ceiling)
            // self.borrowed_mul
        )
        ...
        with boa.env.prank(user):
            ...
            try:
                ...
                if (
                    c_amount * self.collateral_mul // n > 2 * DEAD_SHARES
                    and c_amount * self.collateral_mul // n
                    < (2**128 - 1) // DEAD_SHARES
                ):
                    pass
    ...
    def add_collateral(self, c_amount, user_id):
        ...
        with self.health_calculator(user, c_amount, 0):
            with boa.env.prank(user):
                ...
                ) * self.collateral_mul * self.amm.get_p() > 2**256 - 1:
                    with boa.reverts():
                        self.controller.add_collateral(c_amount, user)
                    raise AllGood()
                ...
                try:
                    ...
                    assert (
                        c_amount + self.amm.get_sum_xy(user)[1]
                    ) * self.collateral_mul > (2**128 - 1) // (50 * DEAD_SHARES)
                    raise AllGood()
        ...
        with self.health_calculator(user, c_amount, amount):
            with boa.env.prank(user):
                ...
                if self.controller.total_debt() + amount > self.debt_ceiling:
                    ...
                    ) * self.amm.get_rate_mul() * self.collateral_mul > 2**256 - 1:
                        with boa.reverts():
                            self.controller.borrow_more(c_amount, amount)
                    else:
                        with boa.reverts():
                ...
                if (
                    final_collateral * self.amm.get_p() * self.collateral_mul
                    > 2**256 - 1
                ):
                    with boa.reverts():
                        self.controller.borrow_more(c_amount, amount)
...
def test_stateful_lendborrow(
    amm, controller, collateral_token, borrowed_token, accounts
):
