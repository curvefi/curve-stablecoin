...
def test_dydx_limits(amm, amounts, accounts, ns, dns, collateral_token, admin, borrowed_token):
    ...
    # Small swap
    dy1, dx1 = amm.get_dydx(0, 1, 10**(collateral_deciamls - 2))
    ...
    dy, dx = amm.get_dydx(1, 0, 10**(collateral_deciamls - 2))  # No liquidity
    assert dx == 0
    assert dy == 0  # Rounded down

    # Huge swap
    dy1, dx1 = amm.get_dydx(0, 1, 10**12 * 10**collateral_deciamls)
    dx2, dy2 = amm.get_dxdy(0, 1, dx1)
    ...
    dy, dx = amm.get_dydx(1, 0, 10**12 * 10**collateral_deciamls)  # No liquidity
    assert dx == 0
    assert dy == 0  # Rounded down
