...
def test_create_loan_within_max_borrowable(
    ...
    approve_zap(borrower, controller, leverage_zap, collateral_token, borrowed_token)
    with boa.env.prank(borrower):
        leverage_zap.create_loan(controller_id, user_collateral, max_b, N, *calldata)
    ...
    with boa.env.prank(borrower):
        with boa.reverts("Debt too high"):
            leverage_zap.create_loan(
                controller_id, user_collateral, exceed_debt, N, *calldata
            )
...
def test_create_loan_exceeds_available_balance_reverts(
    ...
    with boa.env.prank(borrower):
        with boa.reverts():
            leverage_zap.create_loan(
                controller_id, large_collateral, exceed_debt, N, *calldata
            )
...
def test_borrow_more_up_to_create_loan_max(
    ...
    with boa.env.prank(borrower):
        leverage_zap.create_loan(
            controller_id, user_collateral, initial_debt, N, *create_calldata
        )
    ...
    with boa.env.prank(borrower):
        leverage_zap.borrow_more(controller_id, 0, d_debt, *borrow_calldata)
...
def test_borrow_more_debt_too_high_reverts(
    ...
    with boa.env.prank(borrower):
        with boa.reverts("Debt too high"):
            leverage_zap.borrow_more(controller_id, 0, exceed_debt, *calldata)
